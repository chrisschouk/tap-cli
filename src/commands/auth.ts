  ...
  cmd
    ...
    .action(async () => {
      ...
      const supabaseUrl = await clack.text({
        ...
        validate: v => (!v.startsWith('https://') ? 'Must be a valid HTTPS URL' : undefined),
      });
      ...
      const supabaseKey = await clack.password({
        ...
        validate: v => (!v || v.length < 20 ? 'Key looks too short' : undefined),
      });
  ...
  cmd
    ...
    .action(() => {
      ...
      if (config) {
        ...
        out.warn('Not authenticated. Run "tap auth login" or set environment variables.');
      }
    });
